...
func (d *dc) decode(offset uint, result reflect.Value, depth int) (uint, error) {
	if depth > maximumDataStructureDepth {
		return 0, newDbError("exceeded maximum data structure depth; database is likely corrupt")
	}
...
func (d *dc) decodeToDeserializer(offset uint, dser deserializer, depth int, getNext bool) (uint, error) {
	if depth > maximumDataStructureDepth {
		return 0, newDbError(
...
func (d *dc) decodePath(offset uint, path []any, result reflect.Value) error {
PATH:
	for i, v := range path {
		var (
			typeNum writer.K3dt
...
func unmarshalBool(size, offset uint, result reflect.Value) (uint, error) {
	...
	return newOffset, newUnmarshalError(value, result.Type())
}

func indirect(result reflect.Value) reflect.Value {
	for {
		kind := result.Kind()
		if kind == reflect.Interface && !result.IsNil() {
			e := result.Elem()
...
func (d *dc) unmarshalBytes(size, offset uint, result reflect.Value) (uint, error) {
...
func (d *dc) unmarshalFloat32(size, offset uint, result reflect.Value) (uint, error) {
	if size != 4 {
		return 0, newDbError(
...
func (d *dc) unmarshalFloat64(size, offset uint, result reflect.Value) (uint, error) {
	if size != 8 {
		return 0, newDbError("неверный размер секции данных (float 64 = %v)", size)
	}
	...
	case reflect.Float32, reflect.Float64:
		if result.OverflowFloat(value) {
			return 0, newUnmarshalError(value, result.Type())
		}
...
func (d *dc) unmarshalInt32(size, offset uint, result reflect.Value) (uint, error) {
	if size > 4 {
		return 0, newDbError(
...
func (d *dc) unmarshalMap(
	...
	switch result.Kind() {
	default:
		return 0, newUnmarshalStrError("map", result.Type())
	case reflect.Struct:
...
func (d *dc) unmarshalPointer(size, offset uint, result reflect.Value, depth int) (uint, error) {
	pointer, newOffset, err := d.decodePointer(size, offset)
	if err != nil {
		return 0, err
	}
...
func (d *dc) unmarshalSlice(size uint, offset uint, result reflect.Value, depth int) (uint, error) {
	switch result.Kind() {
	case reflect.Slice:
	...
	return 0, newUnmarshalStrError("array", result.Type())
}

func (d *dc) unmarshalString(size, offset uint, result reflect.Value) (uint, error) {
...
func (d *dc) unmarshalUint128(size, offset uint, result reflect.Value) (uint, error) {
	if size > 16 {
		return 0, newDbError(
...
func (d *dc) decodeBytes(size, offset uint) ([]byte, uint) {
...
func (d *dc) decodeFloat64(size, offset uint) (float64, uint) {
	newOffset := offset + size
	bits := binary.BigEndian.Uint64(d.buffer[offset:newOffset])
	return math.Float64frombits(bits), newOffset
}

func (d *dc) decodeFloat32(size, offset uint) (float32, uint) {
	newOffset := offset + size
	bits := binary.BigEndian.Uint32(d.buffer[offset:newOffset])
	return math.Float32frombits(bits), newOffset
}

func (d *dc) decodeInt(size, offset uint) (int, uint) {
	newOffset := offset + size
	var val int32
...
func (d *dc) decodeMapToDeserializer(
	size uint,
	offset uint,
	dser deserializer,
	depth int,
...
func (d *dc) decodePointer(size uint, offset uint) (uint, uint, error) {
...
func (d *dc) decodeSliceToDeserializer(
	size uint,
	offset uint,
	dser deserializer,
	depth int,
...
func (d *dc) decodeString(size, offset uint) (string, uint) {
	newOffset := offset + size
	return string(d.buffer[offset:newOffset]), newOffset
}

func (d *dc) decodeStruct(size uint, offset uint, result reflect.Value, depth int) (uint, error) {
...
func (d *dc) decodeUint(size, offset uint) (uint64, uint) {
	newOffset := offset + size
...
func (d *dc) decodeUint128(size, offset uint) (*big.Int, uint) {
	newOffset := offset + size
	val := new(big.Int)
...
// using unsafe.
func (d *dc) decodeKey(offset uint) ([]byte, uint, error) {
...
func (d *dc) nextValueOffset(offset, numberToSkip uint) (uint, error) {
	if numberToSkip == 0 {
		return offset, nil
	}
	typeNum, size, offset, err := d.decodeCtrlData(offset)
