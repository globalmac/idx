...
> Важно: в активной разработке, использовать только для тестов + скоро добавлю больше примеров использования + переход на ключи Uint64

### Особенности: 

- Без сторонних зависимостей
- Используется B-tree с UINT32 ключём и значениями в виде Slice и Map (см. ниже примеры)
...
- Идеально подходит для формирования поисковой БД с ключами, например в виде числовом представлении строкового значения (аккуратно с коллизиями)
...
- За ключами (Uint32) необходимо следить самостоятельно (в случае повтора - данные перезапишутся)
...
db, err := writer.New(
    writer.Config{
        NodeSize: 32, // В зависимости от кол-ва записей: 24 (файл до 50Mb), 28 (файл до 250Mb) / 32 (файл до 5Gb)
    },
)
if err != nil {
    panic(err)
